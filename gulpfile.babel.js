...
const compileInitCSS = () =>
  gulp.src(config.paths.src.cssInit)
    .pipe(plumber({ errorHandler: notify.onError("Style initial build error: <%= error.message %>") }))
    .pipe(less())
    .pipe(gulpConcat('init.css'))
    .pipe(groupMedia())
    .pipe(autoprefixer(config.options.prefixes, { cascade: true }))
    .pipe(gulp.dest(config.paths.build.cssInit));
...
const watchInitCSS = () => gulp.watch(config.paths.watch.cssInit, gulp.series(compileInitCSS, compileHTML));
...
const buildHTML = gulp.series(compileJSON, compileInitCSS, compileHTML);
const buildStuff = gulp.parallel(compileFavicon, compileFonts, compileJs, compileWebp, compileImages, compileCSS);
const watch = gulp.parallel(watchHTML, watchCSS, watchInitCSS, watchJs, watchImages, watchFonts, watchFavicon, watchSvgIcon);
